...
#              2.选择hive类型数据库，风险等级分别为高，中，低，极高，状态启用
# 预期结果：
...
class HiveSqlRe(unittest.TestCase):
    def setUp(self):
