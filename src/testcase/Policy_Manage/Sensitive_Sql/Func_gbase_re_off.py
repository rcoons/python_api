...
#              2.选择gbase类型数据库，风险等级分别为高，中，低，极高，状态禁用
# 预期结果：
# 1.在gbase的客户端执行sql语句，执行sql语句不被阻断，有审计，显示风险等级安全，匹配到自定义规则
...
class GbaseSqlReOff(unittest.TestCase):
    def setUp(self):
        self.sqllist = commen.PutsqlNum()
        self.ruler_name = commen.PutsqlName("gbase_")
        LOG.info("规则名称：%s" % self.ruler_name)
        LOG.info("SQL语句：%s" % self.sqllist)
        self.sensql = SensitiveSql(self.ruler_name, GlobalConfig.db_type_['gbase'],
                                   dbservice.select_dbservice_byname(gp.run_db['gbase']))
        self.sensitiveway = SensitiveWay()

    @logger('敏感gbase类型低风险禁用')
    def test_gbase_re_off_risk1(self):
        '''敏感gbase类型低风险禁用'''
        self.sensql.sqlrisk_re_off(gp.risk_level['risk_low'], self.sqllist)

    @logger('敏感gbase类型中风险禁用')
    def test_gbase_re_off_risk2(self):
        '''敏感gbase类型中风险禁用'''
        self.sensql.sqlrisk_re_off(gp.risk_level['risk_mid'], self.sqllist)

    @logger('敏感gbase类型高风险禁用')
    def test_gbase_re_off_risk3(self):
        '''敏感gbase类型高风险禁用'''
        self.sensql.sqlrisk_re_off(gp.risk_level['risk_high'], self.sqllist)

    @logger('敏感gbase类型极高风险禁用')
    def test_gbase_re_off_risk4(self):
        '''敏感gbase类型极高风险禁用'''
