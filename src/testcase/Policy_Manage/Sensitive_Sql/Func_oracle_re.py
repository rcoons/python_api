...
#              2.选择oracle类型数据库，风险等级分别为高，中，低，极高，状态启用
# 预期结果：
# 1.在oracle的客户端执行sql语句，执行sql语句被阻断（低风险不被阻断），有审计，显示风险等级，匹配到自定义规则
...
class OracleSqlRe(unittest.TestCase):
    def setUp(self):
