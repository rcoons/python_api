...
#              2.选择sqlserver类型数据库，风险等级分别为高，中，低，极高，状态启用
# 预期结果：
# 1.在sqlserver的客户端执行sql语句，执行sql语句被阻断（低风险不被阻断），有审计，显示风险等级，匹配到自定义规则
...
class SqlserverRe(unittest.TestCase):
    def setUp(self):
    ...
    @logger('敏感sqlserver类型低风险')
    def test_sqlserver_re_risk1(self):
        '''敏感sqlserver类型低风险'''
        self.sensql.sqlrisk_re(gp.risk_level['risk_low'], self.sqllist, gp.risk_info[1])

    @logger('敏感sqlserver类型中风险')
    def test_sqlserver_re_risk2(self):
        '''敏感sqlserver类型中风险'''
        self.sensql.sqlrisk_re(gp.risk_level['risk_mid'], self.sqllist, gp.risk_info[2])

    @logger('敏感sqlserver类型高风险')
    def test_sqlserver_re_risk3(self):
        '''敏感sqlserver类型高风险'''
        self.sensql.sqlrisk_re(gp.risk_level['risk_high'], self.sqllist, gp.risk_info[3])

    @logger('敏感sqlserver类型极高风险')
    def test_sqlserver_re_risk4(self):
        '''敏感sqlserver类型极高风险'''
